...
class IBinder : virtual public BAtom
{
	public:
				...
				/*!	The \a bindings is a mapping of keys that will get pushed on this
					...
					e.g.
						...
						- The following will get called on binder2:
							binder2->Effect(BValue("B", "C"), BValue::wild, BValue::undefined, NULL);
				*/
		...
		virtual	status_t				Effect(	const BValue &in,
												...
												BValue *out) = 0;
				//!	Synonym for Effect(in, BValue::wild, BValue::undefined, NULL).
				...
				//!	Synonym for Effect(BValue::undefined, BValue::undefined, BValue::wild, [result]).
				BValue					Get() const;
				//!	Synonym for Effect(BValue(func,args), BValue::wild, func, [result]).
				BValue					Invoke(const BValue &func, const BValue &args);
				//!	Synonym for Effect(BValue(func,BValue::wild), BValue::wild, func, [result]).
				BValue					Invoke(const BValue &func);
		...
		virtual	status_t				Transact(	uint32 code,
				...
				//!	Return true if this binder still existed as of the
