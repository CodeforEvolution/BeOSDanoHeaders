...
inline	coord				Cofactor(element e) const;
...
static	B2dTransform		MakeReflect(const coord radians);
		...
		B2dTransform&		Reflect(const coord radians);
...
const ITextOutput::sptr&	operator<<(const ITextOutput::sptr& io, const B2dTransform& matrix);
...
inline coord B2dTransform::Cofactor(element e) const
{
	if (e == XX)	return fMatrix[YY];
	if (e == YX)	return -fMatrix[YX];
	if (e == XY)	return -fMatrix[XY];
	if (e == YY)	return fMatrix[XX];
	if (e == XT)	return fMatrix[XY]*fMatrix[YT] - fMatrix[XT]*fMatrix[YY];
	if (e == YT)	return fMatrix[XT]*fMatrix[YX] - fMatrix[XX]*fMatrix[YT];
	return 0;
}
