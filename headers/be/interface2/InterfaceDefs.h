...
struct dimth {
	...
	float to_px(float def, device &d=device::screen) const {
	...
	float to_px(float size, float def, device &d=device::screen) const {
		// The right default to pass to to_nm() below is def/size, but
		// returning the default from to_nm() is a "can't happen" in this
		// case (we eliminate it by calling to_nm() only if is_relative()),
		// so we won't waste time doing the divide.
		if (is_absolute()) return to_px(def, d);
