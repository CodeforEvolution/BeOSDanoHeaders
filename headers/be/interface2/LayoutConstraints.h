...
struct BLayoutConstraint
{
	...
	float Min(float size) const {
		return min.to_px(size, 0);
	}

	float Max(float size) const {
		return max.to_px(size, INFINITY);
	}

	float Pref(float size, float def) const {
		return pref.to_px(size, def);
	}

	float ImplicitSize(dimth param) const {
		float tmp;
	...
	float ImplicitMinSize() const {
		return ImplicitSize(min);
	}

	float ImplicitMaxSize() const {
		if (!(flags & posIsRelativeToStart)) return NAN;
		return ImplicitSize(max);
	}

	float ImplicitPrefSize() const {
		return ImplicitSize(pref);
	}

	void Resolve(float size, float &outPosition, float &outSize) const {
		float min = Min(size);
		float max = Max(size);
		float r = Pref(size, size);
		if (r > max) {
...
struct BLayoutConstraints
{
	...
	BPoint PrefSize(BPoint size) {
		return BPoint(axis[B_HORIZONTAL].Pref(size.x, size.x),
		              axis[B_VERTICAL].Pref(size.y, size.y));
	}
