...
public:
	...
	/*! fetch the value for a given key. given BValue::wild, return the whole map. */
	...
	/*! return an arbitrary description of the control for the given key.
		return BValue::wild if the key is not mapped. */
	virtual	BValue					ControlInfo(const BValue & key) const = 0;

	//! return a snapshot of the map.  equivalent to Control(BValue::wild).
